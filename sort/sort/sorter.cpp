...
//
    ...
    // generate a random vector array of float que va a ser usado para quick sort
    vector<int> arr_d;
    int tamaño5 = 100000;
    for (size_t i = 0; i < tamaño5; i++)
    {
        arr_d.push_back((int)rand() % 250);
        // arr_f.push_back((double)rand() / RAND_MAX);
    }

    cout << "Unsorted:" << endl;
    print_vector(arr_d, 5);

    // create instance of merge sort
    DECLARE_TIMING(d);
    START_TIMING(d);
    QuickSort<int> quicksort;
    quicksort.sort(arr_d);

    cout << endl << "Sorted With quick Sort:" << endl;
    print_vector(arr_d, 25);
    STOP_TIMING(d);
    SHOW_TIMING(d, " quick sort");
